...
class SingleTrackFitWidget(QtWidgets.QWidget):
    ...
    def __init__(self,
        ...
        self.all_points_original_indices: List[int] = [] # Maps plotted point index to original index in track_element_copy['data']
        self.raw_frame_indices_for_plotted_points: List[int] = [] # Stores actual frame number for each plotted point
    ...
    def load_track_data(self,
        ...
        # Load analysis settings from the track_element_copy
        ...
        # Prepare plot data (y_plot vs t_seconds)
        ...
        if not track_point_data_list:
            ...
            self.clear_and_disable() # Ensure UI reflects no data
            if self.track_id_label: self.track_id_label.setText(f"<b>Track ID: {self.track_id or 'N/A'} (No Data)</b>") # Still show ID
            return
        ...
        for original_idx, point_data_tuple in enumerate(track_point_data_list):
            ...
            y_plot = float(self.video_height) - y_tl_px # Convert to bottom-up
            self.plot_data_y_vs_t.append((t_seconds, y_plot))
            self.all_points_original_indices.append(original_idx) # Store original index from 'data' list
        ...
        self._fit_parabola() # Perform initial fit
    ...
    def _fit_parabola(self) -> None: # [cite: 95]
    ...
    def _plot_fitted_curve(self) -> None: # [cite: 95]
        ...
        if self.fit_coeffs and self.plot_data_y_vs_t:
            ...
            if not plot_times and self.plot_data_y_vs_t: # Fallback to all data range if no valid fit_time_range_s
    ...
    def _update_point_visuals(self) -> None: # [cite: 95]
    ...
    def _get_current_analysis_state_dict(self) -> Dict[str, Any]: # [cite: 95]
        excluded_frames_list: List[int] = []
        # Ensure track_element_copy and its data exist before accessing
        track_data_points = []
        ...
        current_g = self.current_g_value_ms2 # Already updated by _on_g_value_changed or load_track_data
        
        is_applied = False # Default
        ...
        state_dict = {
            ...
            'fit_results': {
                ...
                'is_applied_to_project': is_applied # Preserve this flag unless explicitly changed
            }
        }
        return state_dict

    @QtCore.Slot()
    def _on_refit_button_clicked(self) -> None: # [cite: 97]
    ...
    def _on_g_value_changed(self) -> None:
        if self.g_input_lineedit:
            try:
                ...
                if new_g > 0:
                    if abs(self.current_g_value_ms2 - new_g) > 1e-6: # Only update if changed significantly
                        self.current_g_value_ms2 = new_g
                        logger.info(f"User changed g value to: {self.current_g_value_ms2} m/s^2. Re-fitting advised.")
                        # Optionally, you could automatically trigger a re-fit here:
                        # self._fit_parabola()
                else: # Revert if non-positive
                    self.g_input_lineedit.setText(str(self.current_g_value_ms2))
            except ValueError: # Revert if not a float
                self.g_input_lineedit.setText(str(self.current_g_value_ms2))
        # The actual fit will use self.current_g_value_ms2 when _fit_parabola is called.

    @QtCore.Slot()
    def _on_save_analysis_button_clicked(self) -> None: # [cite: 96]
        if self.track_id is not None:
            current_state = self._get_current_analysis_state_dict()
            # Preserve the 'is_applied_to_project' status from the original loaded state
            # as this button only saves settings/results, not the application status.
            if self.track_element_copy and 'analysis_state' in self.track_element_copy:
            ...
            else: # Should not happen if track_element_copy is always populated by load_track_data
    ...
    @QtCore.Slot()
    def _on_apply_scale_button_clicked(self) -> None: # [cite: 97]
    ...
    def _on_reset_fit_settings_button_clicked(self) -> None:
        ...
        # Reset included points mask to all true
        self.included_point_indices_mask = [True] * len(self.plot_data_y_vs_t)
        ...
        # Reset g to default if it was modified (optional, or keep user's g)
        # For now, let's keep the user-set g unless they also want a "Reset g" button.
        # self.current_g_value_ms2 = DEFAULT_ANALYSIS_STATE['fit_settings']['g_value_ms2']
    ...
    def _on_point_clicked(self, scatter_plot_item: pg.ScatterPlotItem, clicked_points: List[pg.SpotItem]) -> None: # [cite: 95]
        ...
        point_data_index = clicked_spot.data() # This is the index in self.plot_data_y_vs_t
    ...
    @QtCore.Slot(object)
    def _on_time_range_changed(self, region_item: pg.LinearRegionItem) -> None: # [cite: 95]

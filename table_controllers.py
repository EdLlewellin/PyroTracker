...
class TrackDataViewController(QtCore.QObject):
    ...
    def __init__(self,
                 ...
                 element_manager: 'ElementManager', # Updated type hint
        ...
        self._element_manager = element_manager # Updated attribute name
    ...
    def handle_visibility_header_clicked(self, logical_index: int) -> None:
        ...
        if target_mode:
            if config.COL_VIS_HIDDEN <= logical_index <= config.COL_VIS_ALWAYS : 
    ...
    def _on_track_selection_changed_in_table(self) -> None:
        ...
        # --- MODIFICATION: Ensure row is valid before accessing item ---
    ...
    def _on_tracks_table_cell_clicked(self, row: int, column: int) -> None:
        if not self._video_loaded:
            return
        # --- MODIFICATION: Ensure row is valid before accessing item (especially for last button row if it still exists temporarily) ---
        if row < 0 or row >= self._tracks_table.rowCount():
             logger.debug(f"TrackDataViewController: Cell click on invalid row {row}. Ignoring.")
             return
        # If the last row was the button row, clicking it might not have an id_item.
        # This check becomes more important after removing the button row.
        # --- END MODIFICATION ---

        id_item = self._tracks_table.item(row, config.COL_TRACK_ID)
        if not id_item:
            # This might happen if clicking on the (now removed) button row or an empty part of the table
        ...
        if is_ctrl_click:
            if is_active_element_in_manager:
                ...
                logger.info(f"TrackDataViewController: Ctrl+Clicked on non-active TRACK element {element_id_clicked}. Selecting.")
    ...
    def _select_element_row_by_id_in_ui(self, element_id_to_select: int,
        ...
        found_row = -1
        # --- MODIFICATION: Iterate only over actual data rows ---
        num_data_rows_in_table = 0
        if target_table is self._tracks_table:
            # Count only rows that represent actual tracks, excluding the button row if it's still there (it shouldn't be after this fix)
            # This assumes track_summary_list accurately reflects data rows.
            num_data_rows_in_table = len(self._element_manager.get_track_elements_summary())
        elif self._lines_table and target_table is self._lines_table:
            num_line_elements = sum(1 for el in self._element_manager.elements if el['type'] == ElementType.MEASUREMENT_LINE)
            num_data_rows_in_table = num_line_elements
        else: # Fallback or for other tables in future
        ...
        if found_row != -1:
            ...
            target_table.clearSelection()
            logger.debug(f"Controller: Could not find row for element ID {element_id_to_select} in table {target_table.objectName()}.")
    ...
    def update_tracks_table_ui(self) -> None:
        ...
        # Store previous count of actual data rows for comparison
        previous_track_data_count = 0
        for r in range(self._tracks_table.rowCount()):
            id_item = self._tracks_table.item(r, config.COL_TRACK_ID)
            # Ensure it's a data row, not a potential button row (which shouldn't exist after this fix)
            if id_item and id_item.data(QtCore.Qt.ItemDataRole.UserRole) is not None:
                previous_track_data_count +=1
        ...
        # --- MODIFICATION: No longer add +1 for button row ---
        ...
        for row_idx in range(num_data_rows):
            self._tracks_table.setSpan(row_idx, 0, 1, 1) # Reset span for safety, though not strictly needed after button removal
        ...
        # --- MODIFICATION: Remove dynamic button creation ---
        # The button_row_index logic and setCellWidget/setSpan for the button are removed.
        # --- END MODIFICATION ---
        ...
        # --- MODIFICATION: Compare num_data_rows with previous_track_data_count ---
        if num_data_rows != previous_track_data_count:
        # --- END MODIFICATION ---
    ...
    def update_lines_table_ui(self) -> None:
        ...
        COL_LINE_ID = config.COL_LINE_ID       
        COL_LINE_FRAME = config.COL_LINE_FRAME    
        ...
        if self._lines_table.columnCount() > 0:
            ...
            if self._lines_table.columnCount() > COL_LINE_ID: header.setSectionResizeMode(COL_LINE_ID, QtWidgets.QHeaderView.ResizeMode.ResizeToContents) 
            if self._lines_table.columnCount() > COL_LINE_FRAME: header.setSectionResizeMode(COL_LINE_FRAME, QtWidgets.QHeaderView.ResizeMode.ResizeToContents) 

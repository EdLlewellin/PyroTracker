...
class TrackDataViewController(QtCore.QObject):
    ...
    def __init__(self,
        ...
        # --- NEW: Connect trackListChanged to update_lines_table_ui ---
        if self._lines_table: # Only connect if the table exists
            self._track_manager.trackListChanged.connect(self.update_lines_table_ui)
        # --- END NEW ---
        self._track_manager.activeTrackDataChanged.connect(self.update_points_table_ui)
        self._track_manager.activeTrackDataChanged.connect(self._sync_tracks_table_selection_with_manager) 
        ...
        if id_item:
            ...
            if element_id is not None and isinstance(element_id, int):
                ...
                if element_index_in_manager != -1:
                    ...
                    logger.warning(f"Could not find TRACK element with ID {element_id} in TrackManager from table selection.")
    ...
    def _sync_tracks_table_selection_with_manager(self) -> None:
        ...
        active_type = self._track_manager.get_active_element_type()
    ...
    def update_tracks_table_ui(self) -> None:
        ...
        current_active_type = self._track_manager.get_active_element_type()
    ...
    def update_points_table_ui(self) -> None:
        ...
        # --- NEW: Display points if active element is a MEASUREMENT_LINE ---
            ...
            active_points = self._track_manager.elements[self._track_manager.active_element_index]['data'] 
        # --- END NEW ---
        else:
